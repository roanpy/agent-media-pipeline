...
The default transcode container comes from the profile that `defaultProfiles.tv|movie` selects (MP4 in the example and current deployment). Transcode profiles support only `mp4` and `mkv`; other container names are rejected during config validation. A profile with `container: "mkv"` keeps the source's embedded subtitle streams, per-track language/title metadata, and Matroska attachments such as ASS fonts during transcode; MP4 still drops embedded subtitles/attachments for compatibility, while same-stem external subtitle files survive both modes. The example includes `movie1080-mkv` and `tv1080-mkv`; existing private configs may not contain newly added example profiles, so run `profiles` first.
...
The default Plex TV template appends a verified per-episode title when available: `Show - S01E03 - Episode title.ext`. With no reliable title it keeps `Show - S01E03.ext`. The same title and episode-specific provider ID are written to the sibling episode NFO; `tvshow.nfo` remains show-level and does not embed the episode catalogue. Metadata lookups are opt-in: `metadata.provider: "none"` disables all provider requests, and omitting the metadata block does not trigger an implicit TVMaze request. `provider: "tmdb"` retains the TVMaze fallback by default; an omitted provider needs an explicit `tvFallback: "tvmaze"` to enable it. TMDB, TVMaze fallback, Jackett, and Prowlarr are optional; missing credentials do not block direct/local/web ingest.

`repair` accepts exactly one existing show folder and emits a JSON preview by default; only explicit `--apply` changes files. It supports Season 0 and carries same-stem subtitles, episode images, and NFO with the media. Missing reliable titles preserve original paths, and `tvshow.nfo` is never changed. Every new path is copied and byte-verified before the old path is removed, so interruption can leave duplicates but not lose media. Multi-episode files such as `S01E01-E02` must still be split first. `--metadata` requires an existing regular JSON file; missing paths fail immediately.
...
- Archive conflict preflight prevents a late artwork conflict from leaving a task partially delivered; an output containing only NFO/artwork/subtitles is rejected. For incremental TV episodes, `--merge` keeps existing different artwork (show-level poster/fanart/banner/clearlogo, episode thumbnails, season posters) and `tvshow.nfo` while media, subtitle, and episode-NFO conflicts still fail.
...
- `downloadRetries` controls finite native network retries (`0-10`, default `3`; `0` disables them). Counts are per aria2 request or yt-dlp HTTP/fragment/extractor request, not a whole playlist; unavailable yt-dlp fragments abort the download. Existing task timeouts still apply, and the pipeline does not switch sources automatically.
- `doctor` reports tool versions and yt-dlp's Deno/EJS inventory without contacting sites. `doctor --online` explicitly probes configured TMDB `/configuration` and enabled Torznab `t=caps` endpoints (10s each), unless offline mode is set; readable caps do not prove download availability. `doctor --cookies SPEC` checks a private file's permissions or accepts a browser specification; neither proves a working login. YouTube setup follows the upstream [EJS guide](https://github.com/yt-dlp/yt-dlp/wiki/EJS).
...
- Failed local retries fingerprint same-stem subtitle and conventional artwork siblings; after changing one, use `--reset-work` only after confirming the new source.
...
When adding S02 with `--merge`, existing `tvshow.nfo`, poster, and fanart remain unchanged, and an existing different episode thumbnail or season poster is kept instead of failing the task; the pipeline adds new episode NFO/artwork/subtitles and the season poster when that slot is free. Plex does not require rewriting show-level metadata for a new season, and `season.nfo` is optional. Legacy root `thumb.png` is not maintained as a Plex-standard asset.
