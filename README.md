...
- Archive conflict preflight prevents a late artwork conflict from leaving a task partially delivered; an output containing only NFO/artwork/subtitles is rejected. For incremental TV episodes, `--merge` keeps existing different show-level artwork/`tvshow.nfo` while media conflicts still fail.
