...
1. **Baseline/version**: `./run.sh --version` prints `Agent Media Pipeline 0.4.0 (config schema 1)`; `./scripts/smoke-test.sh` prints `integration test passed`; `ruff check media-downloader.py tests/test_pipeline.py` clean; `python3 -m py_compile` both files OK.
...
10. **doctor**: `./run.sh doctor` returns valid JSON with `version: 0.4.0` and `configSchemaVersion: 1`; `download:output` is `ok` when `downloadDir` is set.
...
16. **Episode thumbnails**: a TV adopt whose episode metadata supplies `thumbPath` (or a stubbed TMDB still) must produce `Show - SxxEyy - Title-thumb.jpg` beside the episode; missing stills only warn and never fail the task.
17. **Subtitles**: a stubbed yt-dlp acquire with `--write-subs --sub-langs zh-CN,en` must include `--write-subs --write-auto-subs --sub-format srt/best --sub-langs zh-CN,en`; invalid language codes are rejected; non-yt-dlp sources with `--write-subs` fail with a clear error. External same-stem `.srt` files must be carried through both organize and transcode.
18. **MKV transcode keeps embedded subtitles**: a movie profile with `container: "mkv"` transcoding a fixture with an embedded `srt` stream must produce an `.mkv` whose streams include `subtitle` (no `-sn`); the MP4 profile still drops embedded subtitles and keeps external sidecars.
